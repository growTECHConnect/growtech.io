...
class News extends React.Component {
            ...
            <div className="row">
                <div className="col-sm-12">
                    <div className="parent">
                        <h3>{item.title}</h3>
                        <p dangerouslySetInnerHTML={this.renderMarkup(item.body)}></p>
                        <span>{date}</span>
                    </div>
                </div>
            </div>
    ...
    renderNewsItems = () => {
        ...
        return news.map((item, index) => {
            ...
            const date = moment(item.date).format('ll');
            const style = {
                borderBottom: index + 1 === news.length ? '1px solid #6F6F6F' : null,
            };
                ...
                >
                    ...
                    >
                        <Link to={`/news/${index}`}>
                            <div className="parent">
                                <h3>{item.title}</h3>
                                <p dangerouslySetInnerHTML={this.renderMarkup(body)}></p>
                                <span>{date}</span>
                            ...
                            <div className="row">
                                <div className="col-sm-12">
                                    <div className="breadcrumb">
                                        <Link to="/news"><span className="back_arrow"><img src="/images/breadcrumb_arrow.png"/></span>Back to All News</Link>
                                    </div>
                                </div>
                            </div>
